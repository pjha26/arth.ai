...
import { LeadSchema, Step1Schema, Step2Schema, Step3Schema, industryOptions, companySizeOptions } from "@/lib/validation";
import type { Lead } from "@/lib/validation";

type FormData = Partial<Lead>;
type FieldErrors = Partial<Record<keyof Lead, string>>;
const STEP_LABELS = ["About you", "Your company", "Your challenge"];
        ...
        <div style={{ width: "100%", maxWidth: "500px" }}>
          ...
          <div className="animate-fade-up" style={{ marginBottom: "1.75rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1rem" }}>
              <span className="badge badge-neutral" style={{ fontFamily: "var(--font-heading)" }}>Step {step} of 3</span>
              <span style={{ fontSize: "0.78rem", color: "var(--text-muted)", fontFamily: "var(--font-heading)" }}>— {STEP_LABELS[step - 1]}</span>
            </div>
            <h1 style={{ fontFamily: "var(--font-heading)", fontSize: "1.6rem", fontWeight: 800, letterSpacing: "-0.025em", marginBottom: "0.4rem", color: "var(--charcoal)" }}>
              {step === 1 && "Tell us who you are."}
              {step === 2 && "Tell us about your company."}
              {step === 3 && "What's your biggest challenge?"}
            </h1>
            <p style={{ fontSize: "0.875rem", color: "var(--text-muted)", fontFamily: "var(--font-body)" }}>
          ...
          {/* Card */}
          <div className="card-flat animate-fade-up delay-100" style={{ padding: "2rem", boxShadow: "var(--shadow-md)" }}>

            {/* Step dots + progress */}
            <div style={{ marginBottom: "2rem" }}>
              <div className="step-indicator" style={{ marginBottom: "0.8rem" }}>
                {[1, 2, 3].map((s, i) => (
            ...
            {step === 3 && (
              <div style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
                <div className="input-group">
                  <label className="input-label" htmlFor="painPoints">Biggest operational challenge right now *</label>
                  <textarea id="painPoints" className={`input-field ${err("painPoints") ? "error" : ""}`} placeholder="E.g. We spend too much time on manual follow-ups. Our team is growing but we can't scale without adding headcount..." value={formData.painPoints || ""} onChange={e => update("painPoints", e.target.value)} rows={5} autoFocus />
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    {err("painPoints") ? <span className="input-error-msg">{err("painPoints")}</span> : <span />}
                    <span style={{ fontSize: "0.72rem", color: "var(--text-muted)", fontFamily: "var(--font-heading)" }}>{formData.painPoints?.length || 0}/1000</span>
                  </div>
                </div>
                {/* Preview */}
                <div style={{ padding: "1rem 1.1rem", background: "var(--saffron-50)", border: "1px solid var(--saffron-100)", borderRadius: "var(--r-md)" }}>
                  <p style={{ fontFamily: "var(--font-heading)", fontSize: "0.72rem", color: "var(--saffron-600)", fontWeight: 700, marginBottom: "0.5rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>📄 What you'll receive</p>
                  <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                    {["AI Readiness Score (Digital, Automation, Growth)", "Personalized pain point analysis", "3–5 specific AI automation opportunities", "Actionable next steps for your industry"].map(i => (
                      <li key={i} style={{ fontSize: "0.8rem", color: "var(--text-secondary)", display: "flex", gap: "0.5rem", fontFamily: "var(--font-body)" }}>
            ...
            {/* Submit error */}
            {submitError && (
              <div style={{ marginTop: "1rem", padding: "0.85rem 1rem", background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: "var(--r-md)", fontSize: "0.85rem", color: "#DC2626", fontFamily: "var(--font-body)" }}>
                ⚠️ {submitError}
              </div>
            )}
            ...
            <div style={{ marginTop: "2rem", display: "flex", justifyContent: "space-between", gap: "1rem" }}>
              ...
              {step < 3
                ...
                : (
                  <button id="submit-audit-btn" className="btn btn-saffron" onClick={handleSubmit} disabled={isSubmitting} style={{ opacity: isSubmitting ? 0.7 : 1, cursor: isSubmitting ? "not-allowed" : "pointer", minWidth: "180px", justifyContent: "center" }}>
                    {isSubmitting
                      ? <><span style={{ width: 13, height: 13, border: "2px solid rgba(255,255,255,0.35)", borderTopColor: "white", borderRadius: "50%", display: "inline-block", animation: "spin-cw 0.7s linear infinite", flexShrink: 0 }} /> Generating…</>
                      : "Generate My Audit →"
                    }
                  </button>
                )
          ...
          <p style={{ textAlign: "center", fontSize: "0.72rem", color: "var(--text-muted)", marginTop: "1.25rem", fontFamily: "var(--font-heading)" }}>
            Your data is used only to generate your report. No spam, ever.
          </p>
        </div>
      </div>
    </div>
